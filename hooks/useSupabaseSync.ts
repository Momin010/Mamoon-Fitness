...
import type { Task, Meal, WorkoutSession, Exercise, Friend } from '../types';
  ...
  const fetchWorkouts = useCallback(async () => {
    ...
    for (const session of sessions || []) {
      ...
      workouts.push({
        ...
        exercises: (exercises || []).map((e: any): Exercise => ({
          id: e.id,
          name: e.name,
          sets: e.sets,
          reps: e.reps,
